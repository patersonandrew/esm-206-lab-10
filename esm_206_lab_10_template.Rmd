...
env_var <- read_csv("world_env_vars.csv") %>% 
  na.omit #this is how you omit all NAs, but we should not do this automatically
...
DonnerTable <- read_csv("DonnerTable.csv") #Allison recommends explicitly coercing column with outcome variable to 0 and 1
...
1 = survival, 0 = death
```{r blr}
#glm means something on the right is linearly related to something on the left
donner_blr <- glm(Outcome ~ Sex + Age, family = "binomial", data = DonnerTable)
summary(donner_blr)

#log odds are hard for audience to understand, so we will chnage that
```

c. Use the model to answer: What are the log odds of survival for a 5 year-old female? The probability of survival?

#For predict, the data frame you create must have exact same names that model is looking for.
```{r predict_f_5}
...
```{r new_data}
#This will be useful on the final
...
ggplot(graph_data, aes(x = Age, y = Probability))+
  geom_line(aes(color = Sex))+ #designates that each sex sequence is a distinct group
...
```{r solar_data}
#make a list of any file names that start this way
...
aq_tidy <- aq_df %>% 
  ...
  gather(year, aq_prod, `1960`:`2016`) %>% #now it's tidy rectangular format
  filter(year >= "1990") %>% 
...
aq_plot <- ggplot(aq_tidy, aes(x = as.numeric(year), y = aq_mil, group = country)) +
  ...
  geom_text(aes(label = country, color = country), position = position_nudge(y = 0.04, x = 1))+
